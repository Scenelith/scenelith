# Support

Use [GitHub Discussions](https://github.com/Scenelith/scenelith/discussions) for setup questions and reproducible usage problems. Use [GitHub Issues](https://github.com/Scenelith/scenelith/issues/new/choose) for confirmed defects and focused feature requests.

Include the Scenelith version, host architecture, Docker and Compose versions, relevant redacted logs, and the output of `npm run selfhost:doctor`.

Never post `.env` files, provider keys, session cookies, database dumps, private media URLs, or user content. Vulnerabilities belong in a private security advisory as described in `SECURITY.md`.

The source-available self-hosted edition is community supported and has no response-time guarantee. Operators remain responsible for their infrastructure, provider accounts, backups, updates, and incident response.
