...
const noop = () => {};
function Fixture() {
  const foreground = useRef(new Map<string, string | null>());
  ...
  const apply = (status: BackgroundTaskRecord["status"], minute: number) => {
    ...
    setGraph((current) => ({ ...current, nodes: current.nodes.map((node) => canRestoreForegroundTask(task, foreground.current) ? restoreGeneratorTask(node, task) : node) }));
  };
