...
## Personal consent and team access

Each person connects with their own signed-in Scenelith account. The consent screen identifies that account and offers **Use another account**, preserving the verified agent callback and PKCE request while ending the current browser session. A team member can connect independently after receiving canvas access; the team owner does not approve the MCP connection.

Consent shows available workspaces, canvases and actions using the current edition's role policy. Choosing a workspace or specific canvases updates the available actions. The server rechecks the selection and intersects requested scopes with current permissions when approving, including when the form is modified or access changes while it is open. Canvas creation and workflow publishing are described separately from member editing rights; credential administration is offered only where the role allows it. Workspace permissions remain authoritative on every tool call. Revoking a canvas grant therefore removes the agent's access without requiring a new connection.

### Video Master scene generation
