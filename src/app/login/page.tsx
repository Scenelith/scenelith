...
const oauthErrors: Record<string, string> = {
  ...
  verification_invalid: "This confirmation link is invalid or has expired.",
};
...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ error?: string; reset?: string; mode?: string; next?: string; email?: string; invite?: string }> }) {
  ...
  const teamInvite = params.invite === "1";
  const registrationEnabled = teamInvite || runtimeConfig.registrationMode === "open" || accountCount === 0;
  return <AuthSectionTwo registrationEnabled={registrationEnabled} googleEnabled={Boolean(process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET)} initialMode={registrationEnabled && params.mode === "register" ? "register" : "login"} initialEmail={teamInvite ? String(params.email || "").slice(0, 254) : ""} teamInvite={teamInvite} lastAuthMethod={lastAuthMethod === "google" ? "google" : lastAuthMethod === "email" ? "email" : null} initialError={params.error ? oauthErrors[params.error] || "Could not sign in" : ""} initialNotice={params.reset === "1" ? "Password updated. Sign in with your new password." : ""} returnTo={returnTo} />;
}
