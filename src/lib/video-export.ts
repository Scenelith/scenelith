...
export type VideoMasterExportAudioMode = "selected" | "original" | "none";
...
const mediaRangeSchema = z.object({
  id: z.string().uuid(),
  start: z.number().finite().min(0),
  end: z.number().finite().positive(),
}).refine((asset) => asset.end > asset.start, "Invalid media range");
...
export const videoMasterExportRequestSchema = z.object({
  ...
  audioMode: z.enum(["selected", "original", "none"]).default("selected"),
  assets: z.array(z.object({
    id: z.string().uuid(),
    start: z.number().finite().min(0),
    end: z.number().finite().positive(),
    audio: mediaRangeSchema.optional(),
  }).refine((asset) => asset.end > asset.start, "Invalid video range")).min(1).max(30),
}).refine((request) => request.audioMode !== "original" || request.assets.every((asset) => asset.audio), "Original audio is missing for a scene");
