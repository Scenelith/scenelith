import { automationCanvasReferences, type AutomationReferenceAsset } from "./canvas-references";
...
async function addToCanvas(execution: AutomationNodeExecution, planNoteMode: "truncate" | "preserve" = "truncate") {
  ...
  const referenceIds = [...new Set(items.flatMap((item) => item.referenceAssetIds))];
  const referenceAssets = referenceIds.length ? await db.prepare(`SELECT a.id, a.persona_id, a.role, p.name AS persona_name
    FROM assets a LEFT JOIN personas p ON p.id = a.persona_id AND p.workspace_id = a.workspace_id
    WHERE a.workspace_id = ? AND a.id = ANY(?::text[])`).all(execution.context.workspaceId, referenceIds) as AutomationReferenceAsset[] : [];
  ...
  await mutate((graph) => {
    ...
    for (const [position, item] of items.entries()) {
      ...
      const sourceScene = item.referenceAssetIds.some((id, index) => id === sourceAssetId && item.referenceRoles[index] === "reference-image")
        ? nodes.find((node) => node.data.kind === "scene" && String(node.data.assetId || "") === sourceAssetId)
        : undefined;
      ...
      const output: FrameNode = {
        ...
        data: {
          ...
          attachedReferences: automationCanvasReferences(item, referenceAssets, sourceScene ? sourceAssetId || undefined : undefined),
          status: "ready",
