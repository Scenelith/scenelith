...
const helpByType: Record<string, AutomationNodeHelp> = {
  ...
  "output.add-to-canvas": {
    ...
    technicalNotes: ["Terminal canvas side effect that creates generated-image nodes and lineage links from the connected source when present.", "The selected layout determines placement; Show the plan determines whether a generated plan note is added.", "Result nodes retain the exact request reference asset IDs, input roles and labels. Identity references are attached individually, including their Before/After group; only a source image actually used by generation gets an input connection.", "Test and single-step preview runs return a preview receipt and never change the content canvas.", "Emits a canvas-result receipt but does not accept outgoing workflow connections."],
  },
  "output.finish": {
